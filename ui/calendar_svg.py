...
def calendar_svg(result: dict[str, Any], lang: str = MS) -> tuple[str, str]:
    cultures = result["cultures"]
    if not cultures:
        return "", t(lang, "cal.none")
    ...
    out = [f'<svg viewBox="0 0 {width} {height}" role="img" aria-labelledby="cal-title cal-desc" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto">', f'<title id="cal-title">{html.escape(t(lang, "cal.title"))}</title>', f'<desc id="cal-desc">{html.escape(_alternative(cultures, lang))}</desc>', '<rect width="1080" height="100%" fill="#FFFFFF"/>']
    ...
    out.append(f'<text x="10" y="63" fill="#1B2430" font-family="system-ui" font-size="14">{html.escape(t(lang, "cal.tension"))}</text>')
    ...
    if result["horizon_mois"] == 12:
        out.append(f'<text x="160" y="{height-5}" fill="#1B2430" font-family="system-ui" font-size="14">{html.escape(t(lang, "tl.calendar_caption"))}</text>')
    out.append("</svg>")
    return "".join(out), _alternative(cultures, lang)


def _alternative(cultures: list[dict[str, Any]], lang: str = MS) -> str:
    return " ".join(
        t(lang, "cal.alternative", crop=c["culture"], semis=c["calendrier"]["semis"], recolte=c["calendrier"]["recolte_estimee"], debut=c["calendrier"]["stade_critique"]["debut"], fin=c["calendrier"]["stade_critique"]["fin"], jours=c["recouvrement_avec_tension_j"], etat=c["etat"])
        for c in cultures
    )
